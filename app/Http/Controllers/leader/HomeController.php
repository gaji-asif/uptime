...
//use App\Http\Controller\leader\Controller;
...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('leader.home')->with('data',$data);
        //return view('home');
    }
    ...
    public function Downloadresume($id){
        ...
        $url = Storage::disk("s3")->url('leader/employee/get-resume/'.$id);
         return redirect($url);
        exec('wkhtmltopdf '.$url.' '.$tmpFolder.$id.'_'.'resume.pdf');
        $file = $tmpFolder.$id.'_'.'resume.pdf';
        return redirect(Storage::disk("s3")->url('leader/images/resumes/'.$id.'_'.'resume.pdf'));

...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('executive.home')->with('data',$data);
        //return view('home');
    }
    ...
    public function Downloadresume($id){
        ...
        $url = Storage::disk("s3")->url('executive/employee/get-resume/'.$id);
        //return redirect($url);

        exec('wkhtmltopdf'.$url.' '.$tmpFolder.$id.'_'.'resume.pdf');
        $file = $tmpFolder.$id.'_'.'resume.pdf';
        return redirect(Storage::disk("s3")->url('executive/images/resumes/'.$id.'_'.'resume.pdf'));
    ...
    public function getLooseAndWin($status){
        if(Auth::guard('admin')->user()->access_level == 5){
            ...
            $query = DB::table('builds')
                ...
                ->select('builds.*','employee.full_name')
                ->where('builds.status',$status)
                ->where('builds.company_id',Auth::guard('admin')->user()->company_id)
                ->limit(5)
                ->orderBy('id','desc')
                ->get()
                ->toArray();

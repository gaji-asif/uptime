...
<div class="content-wrapper">
    ...
    <div class="card col-md-8">
        <div class="card-body">
            ...
            <form action="{{ url('master/builds/store') }}" method="POST" class="forms-sample"  enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                 ...
                 <div class="form-group {{($builds_data['categories'] ? '' : 'hide')}}">
                    <strong><label for="category">Category *</label></strong>
                    <select class="form-control" name="category" id="category" required>
                        <option value="">select Category</option>
                        @foreach ($builds_data['categories'] as $cat)
...
<script>
       ...
       $( "#category" ).change(function() {
        ...
        $.ajax({
                url: "{{ url('master/build/getsubcategory') }}/"+cat_id,
                type: "post",
                data: {_token:_token},
                success: function(d) {
                    if(d.sub_cat_html){

...
@if ($errors->any())
    ...
    <div class="card col-md-8">
        ...
        @if(isset($challenge))
            <form action="{{ url('master/preset-challenge/update',$challenge->id) }}" method="POST" class="forms-sample"  enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group {{($challenge_data['categories'] ? '' : 'hide')}}">
                    ...
                    <select class="form-control" name="category" id="category" required>
                        ...
                        @foreach ($challenge_data['categories'] as $cat)
                            <option value="{{$cat['id']}}" {{($challenge->category_id == $cat['id'] ? "selected" : "")}}>{{$cat['category_name']}} 
                            </option>
                        @endforeach
                    </select>
                </div>

                <div class="form-group sub_cat {{($challenge_data['subcategory'] ? '' : 'hide')}}">
                    <strong><label for="subcategory">SubCategory *</label></strong>
                    <select class="form-control" name="subcategory" id="subcategory">
                        <option value="">Select Subcategory</option>  
                        @foreach($challenge_data['subcategory'] as $item)
                           <option value="{{$item['id']}}" {{$challenge->subcategory_id == $item['id'] ?"selected":""}}>{{$item['subcategory_name']}} 
                           </option>  
                        @endforeach 
                    </select>
                </div>
        ...
        @else
            <div class="text-center"><h3><i class="fa fa-warning size-40"></i></br> This preset challenge is in use or its over.</h3></div>
        @endif
        </div>
    </div>
        <!-- </div>

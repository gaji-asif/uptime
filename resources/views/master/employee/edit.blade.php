...
<div class="content-wrapper">
    ...
    <div class="card col-md-8">
        <div class="card-body">
            ...
            <form action="{{ url('master/employee/update',$employee->id) }}" method="POST" class="forms-sample" enctype="multipart/form-data">
                @csrf

                <div class="form-group">
                    <strong><label for="full_name">Full Name *</label> </strong>
                    <input type="text" name="full_name" id="full_name" class="form-control" value="{{$employee->full_name}}" placeholder="Full Name" required>
                </div>
                <div class="form-group">
                    <strong><label for="email">Email address *</label></strong>
                    <input type="email" class="form-control" required id="email" value="{{$employee->email}}" readonly>
                </div>
                <div class="form-group">
                    <strong><label for="phone_number">Phone Number *</label> </strong>
                    <input type="text" name="phone_number" id="phone_number" class="form-control" placeholder="Phone Number" value="{{$employee->phone_number}}" required>
                </div>


                <div class="form-group">
                    @if($employee_data['users'])
                    <strong><label for="company_id">Company Name *</label></strong>
                    <select class="form-control emp-company-select" name="company" id="company_id" required>
                    <option value="">select Company</option>
                    @foreach ($employee_data['users'] as $user)
                        <option value="{{$user['id']}}" {{($employee->company_id == $user['id'] ? "selected" : "")}}>{{$user['name']}}</option>
                    @endforeach
                    </select>
                    @else
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="industry" id="industry" required="">
                        ...
                        @foreach ($employee_data['industry'] as $item)
                            <option value="{{$item['id']}}" {{($employee->industry == $item['id'] ? "selected" : "")}}>{{$item['industry_name']}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <strong><label>Profile Pic</label></strong>
                    <div class="relativeDiv">
                        ...
                        <img src="<?php echo $employee->image;?>" onerror="this.src='<?php echo url('images/no_image.png'); ?>'">
                    </div>
                </div>
                @endif
                <div class="form-group">
